...
# define LOAD_KEY_64BIT(data)                vld1q_u8((data))
# define LOAD_KEY_128BIT(data)               vld1q_u8((data))
...
/* w7  = w0 ^ w1 ^ w2 ^ w3 ^ f(w3)           */
# define EXPAND_128BIT_KEY(k, round, k1, t1, t2, rcon)                  \
    t1 = vmovq_n_u32(vgetq_lane_u32(k1, 0));                            \
    t2 = vmovq_n_u32(vgetq_lane_u32(k1, 1));                            \
    t2 = vsetq_lane_u32(0x0000'0000, t2, 0);                            \
    t1 = veorq_u32(t1, t2);                                             \
    t2 = vmovq_n_u32(vgetq_lane_u32(k1, 2));                            \
    t2 = vsetq_lane_u32(0x0000'0000, t2, 0);                            \
    t2 = vsetq_lane_u32(0x0000'0000, t2, 1);                            \
    t1 = veorq_u32(t1, t2);                                             \
    t2 = vmovq_n_u32(vgetq_lane_u32(k1, 3));                            \
    t2 = vsetq_lane_u32(0x0000'0000, t2, 0);                            \
    t2 = vsetq_lane_u32(0x0000'0000, t2, 1);                            \
    t2 = vsetq_lane_u32(0x0000'0000, t2, 2);                            \
    t1 = veorq_u32(t1, t2);                                             \
    t2 = vdupq_n_u32(SUB_WORD(ROT_WORD(vgetq_lane_u32(k1, 3)), rcon));  \
    k1 = veorq_u32(t1, t2);                                             \
    k[round] = vreinterpretq_u8_u32(k1);
...
inline void aes_ni::expand_128bit_key(const uint8_t * const key, u128_t *encskeys, u128_t *decskeys) const noexcept {
  ...
  u128_t t3 = {0};
#elif (_M_ARM == 7)
  uint32x4_t t1 = LOAD_KEY_128BIT(key);
  uint32x4_t t2 = {0};
  uint32x4_t t3 = {0};
#endif
