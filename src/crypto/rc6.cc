...
void rc6::no_intrinsic_encrypt(const uint8_t * const ptext, uint8_t *ctext) const noexcept {
  ...
  BIGENDIAN_32BIT_U8_TO_U128_COPY(ptext, reg);

  reg[1] = reg[1] + subkeys_[0];
  reg[3] = reg[3] + subkeys_[1];

  for (uint32_t round = 0; round < NROUND; ++round) {
  ...
  BIGENDIAN_32BIT_U128_TO_U8_COPY(reg, ctext);
}
...
void rc6::no_intrinsic_decrypt(const uint8_t * const ctext, uint8_t *ptext) const noexcept {
  ...
  BIGENDIAN_32BIT_U8_TO_U128_COPY(ctext, reg);

  reg[2] = reg[1] + subkeys_[43];
  reg[0] = reg[3] + subkeys_[42];
  ...
  reg[3] = reg[0] + subkeys_[1];
  reg[1] = reg[2] + subkeys_[0];

  BIGENDIAN_32BIT_U128_TO_U8_COPY(reg, ptext);
}
...
void rc6::expand_key(uint32_t *key, uint32_t *skeys, const uint32_t ksize) noexcept {
  uint32_t a = 0, b = 0, i = 0, j = 0, v = 0;
  constexpr uint32_t d = 2 * NROUND + 4;
  skeys[0] = P32;

  for (int32_t k = 1; k < 44; ++k) {
    skeys[k] = skeys[k - 1] + Q32; 
  }

  for (int32_t s = 0; s < 133; ++s) {
    a = ROTATE_LEFT32(skeys[i] + a + b, 3);
    b = ROTATE_LEFT32(key[j] + a + b, a + b); 
